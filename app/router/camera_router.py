...
from app.schema.camera_schema import Response, Create, Update
...
@camera_router.post("/", status_code=status.HTTP_201_CREATED, response_model=Response)
...
@camera_router.delete("/{shop_id}/{camera_id}", response_model=Response)
def delete(shop_id: str, camera_id: str, user_id=Depends(get_current_user)):
    camera_controller.delete(shop_id, camera_id, user_id)
    return {"status": 200, "success": True, "message": "camera deleted successfully", "data": None}


@camera_router.get("/{shop_id}/{camera_id}", response_model=Response)
